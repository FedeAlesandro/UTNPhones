package edu.utn.utnPhones.utils;

public class Constants {

    public static final String NOT_FOUND_AREA_CODE = "The area code entered is wrong, it doesn't exist";

    public static final String NOT_FOUND_BILL = "Bill not found";

    public static final String NOT_FOUND_PHONE_LINE = "Phone line not found";
    public static final String ALREADY_EXISTS_PHONE_LINE = "This phone line already exists";

    public static final String USER_NOT_EXIST = "This user doesn't exist";
    public static final String USER_NOT_EXIST_ID = "The user with the specified id doesn't exist";
    public static final String USER_NOT_EXIST_USERNAME = "The user with the specified user name doesn't exist";
    public static final String LOCATION_NOT_EXIST = "The specified location doesn't exist";
    public static final String NOT_ADDED_USER = "The user can't be added";
    public static final String NOT_UPDATED_USER = "Can't update the user";
    public static final String DUPLICATED_USERNAME = "This username is not available";
    public static final String UNAUTHORIZED_USER_HANDLING = "Unauthorized user type handling";
    public static final String LOGIN_FAILED = "Login failed";

    public static final String NOT_FOUND_TARIFF = "Tariff not found";

    public static final String INVALID_USER_TYPE = "Invalid user type";

    public static final String INFRASTRUCTURE_SESSION = "infrastructure";
    public static final String EMPLOYEE_SESSION = "employee";

}
